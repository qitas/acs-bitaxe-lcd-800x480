...
while (true)
    ...
    if (millis() - lastAutoTuneCheck > (firstAutoTune ? 600000 : 30000) || (asicTemp >= 68 && lastAutoTuneCheck > 500))  // 10 min for first check so bitaxe stabilizes, 30 sec after. Instantly if temp is too high
    {
        presetAutoTune();
